# Based on https://github.com/m1cr0man/nixpkgs/blob/1a5b9da089557275db9aa90df40be1a47a710764/nixos/modules/security/acme/default.nix
{
  config,
  lib,
  pkgs,
  ...
  certToConfig = cert: data: let
    ...
    commonOpts = let
      escArg = lib.escapeShellArg;
      escArgs = map lib.escapeShellArg;
    in
      (escArgs [
      ...
      ++ lib.optionals (!data.emailFromEnvironment) (escArgs [
        "--email"
        data.email
      ])
      ++ (escArgs protocolOpts)
      ++ lib.optionals (acmeServer != null) (escArgs [
        "--server"
        acmeServer
      ])
      ++ lib.optionals (data.csr != null) (escArgs [
        "--csr"
        data.csr
      ])
    ...
    in
      lib.concatStringsSep " " (
        commonOpts
        ++ (escArgs ["run"])
        ++ lib.optionals data.ocspMustStaple (escArgs ["--must-staple"])
        ++ escArgs data.extraLegoRunFlags
      );
    renewOpts = let
      escArg = lib.escapeShellArg;
      escArgs = map lib.escapeShellArg;
    in
      lib.concatStringsSep " " (
        ...
        ++ lib.optionals data.ocspMustStaple (escArgs ["--must-staple"])
        ++ escArgs data.extraLegoRenewFlags
      );
    ...
    renewService = lockfileName: {
      ...
      script =
        (
          ...
          EMAIL="${
            if data.emailFromEnvironment
            then "\${LEGO_ACCOUNT_EMAIL:-\${LEGO_EMAIL}}"
            else data.email
          }"
        ...
        [
          {
            assertion =
              cfg.defaults.email
              != null
              || cfg.defaults.emailFromEnvironment
              || lib.all (
                certOpts:
