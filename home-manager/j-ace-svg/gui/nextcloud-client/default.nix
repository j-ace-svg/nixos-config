...
in {
  config = lib.mkIf cfg.enable {
    ...
    systemd.user.services.nextcloud-client.Service.ExecStartPre = lib.getExe (
      pkgs.writeShellApplication {
        name = "nextcloud-client-start-pre";
        text = let
          ...
          '';
        in ''
          cp "${nextcloud-client-config}" -T "${config.home.homeDirectory}/.config/Nextcloud/nextcloud.cfg"
        '';
      }
    );
